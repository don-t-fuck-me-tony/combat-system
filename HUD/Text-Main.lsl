...
ShowChars(integer link,vector grid_pos1, vector grid_pos2, vector grid_pos3, vector grid_pos4, vector grid_pos5) {
  ...
  // llOwnerSay("process");
   ...
   // llOwnerSay("XYT to Prim: " + (string)link);
...
{
    //llOwnerSay(llList2CSV(gXyTextPrims));
    float time;
  ...
  //  llOwnerSay("bank_offset: " + (string)bank_offset + ", bank_length: " + (string)bank_length
  //  + ", bank_highest_dirty: " + (string)bank_highest_dirty);
    integer x=0;
    for (;x < msgLen;x = x + 10)
    {
        ...
        //link = unpack(gXyTextPrims,(i + bank_offset));llOwnerSay((string)link);
...
init()
{
  ...
  //  llOwnerSay("Linkset prims total: " + (string)num_prims);
    string link_name;
    integer bank=0;
   ...
   // llMessageLinked(LINK_THIS, SLAVE_RESET, "" , null_key);
    ...
    @loop;
    {
        ...
        for(x=0;x<=num_prims;++x)
        {
            ...
            list tmp = llParseString2List(link_name, ["-"], []);//if(llList2String(tmp,0) == "xyzzytext"){llOwnerSay(llList2CSV(tmp));}
...
default {
   ...
   on_rez(integer num)
   {
    ...
    link_message(integer sender, integer channel, string data, key id) {
        ...
        if (channel == DISPLAY_STRING) {
            string TextToRender = data;
            integer link;
            if(id == "0"){link = 12;}
            //else if(id == "1"){link = 13;}
            else if(id == "2"){link = 14;}
          //  else if(id == "3"){link = 15;}
            else if(id == "4"){link = 27;}
           // else if(id == "5"){link = 28;}
            else if(id == "6"){link = 37;}
          //  else if(id == "7"){link = 38;}
            else if(id == "8"){link = 47;}
         //   else if(id == "9"){link = 48;}
            else if(id == "8675309") {
                //integer mem = llGetUsedMemory();
                llSetTimerEvent(0.5);
                //llOwnerSay("Text Main Script Memory usage:\nMemory used: " + (string)mem);
            }
          if(link > 11){RenderString(link,TextToRender);link +=1;TextToRender = llGetSubString(TextToRender, 10, 20);RenderString(link,TextToRender);}
 ...
 //           {//it already exists
   ...
   //         }
    ...
    timer()
    {
        if(beat == 0){beat = 1;llMessageLinked(LINK_THIS, 942969, "Hbeat" , "Gauges!");}
        else if(beat == 1){beat = 0;llMessageLinked(LINK_THIS, 942969, "Hbeat2" , "Gauges!");}
    }
